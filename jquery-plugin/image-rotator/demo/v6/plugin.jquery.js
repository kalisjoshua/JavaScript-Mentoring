    ...
    return this                                                         // return the jQuery collection to not break chaining
      .each(function (indx, item) {                                     // create function scope for each plugin on a page
        ...
        function nextImage () {
          ...
          var fn, src = options.folder + "/" + imgList[0];

          if (options.transition in transition) {                       // protect against an invalid transition value in options object
            fn = transition[options.transition];                        // find the defined optional transition function
          } else if ({}.toString.call(options.transition)) {
            fn = options.transition;                                    // check to see if transition property is a function defined by developer
          } else {
            fn = transition.fade;                                       // use fallback (default) transition function
          }
...
$.fn.ready(function () {
  ...
  $(".ISMs.slow")
    .ISMs({
      ...
      "limit"  : 1,
      "speed"  : 1000,
      "transition": function (behind, front, src) {
        behind.attr("src", src);
        front
          .animate({left: "100%", top: "100%"}, 200, function () {
            front.attr("src", src).css({left: 10, top: 10});
          });
      }
    });
});